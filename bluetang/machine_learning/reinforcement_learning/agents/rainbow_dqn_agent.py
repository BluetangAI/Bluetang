from bluetang.machine_learning.reinforcement_learning.agent import Agent

from bluetang.game_frame import GameFrame
from bluetang.game_frame_buffer import GameFrameBuffer

from bluetang.input_controller import KeyboardEvent, MouseEvent

from bluetang.enums import InputControlTypes

from bluetang.logger import Loggers

from bluetang.utilities import bluetangError
...
try:
    ...
    from bluetang.machine_learning.reinforcement_learning.rainbow_dqn.rainbow_agent import RainbowAgent
    from bluetang.machine_learning.reinforcement_learning.rainbow_dqn.replay_memory import ReplayMemory
except ImportError:
    raise bluetangError("Setup has not been been performed for the ML module. Please run 'bluetang setup ml'")
...
# Adapted for bluetang.AI from https://github.com/Kaixhin/Rainbow
...
class RainbowDQNAgent(Agent):
    ...
    def __init__(
        ...
        if len(game_inputs) > 1:
            raise bluetangError("RainbowDQNAgent only supports a single axis of game inputs.")

        if game_inputs[0]["control_type"] != InputControlTypes.DISCRETE:
            raise bluetangError("RainbowDQNAgent only supports discrete input spaces")
