...
from bluetang.input_recorder import InputRecorder

from bluetang.utilities import bluetangError, clear_terminal

from bluetang.enums import InputControlTypes

from bluetang.logger import Loggers

from bluetang.config import config
...
class RecorderAgent(Agent):
    ...
    def __init__(
        ...
        if window_geometry is None or not isinstance(window_geometry, dict):
            raise bluetangError("RecorderAgent expects a 'window_geometry' dict kwarg.")
        ...
        input_recorder_command = "bluetang record_inputs"
    ...
    def generate_actions(self, state, **kwargs):
        if not isinstance(state, GameFrameBuffer):
            raise bluetangError("RecorderAgent 'generate_actions' state should be a GameFrameBuffer")
