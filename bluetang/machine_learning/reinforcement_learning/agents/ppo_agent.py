...
try:
    ...
    from bluetang.machine_learning.reinforcement_learning.ppo.policy import Policy
    from bluetang.machine_learning.reinforcement_learning.ppo.ppo import PPO
    from bluetang.machine_learning.reinforcement_learning.ppo.rollout_storage import RolloutStorage
except ImportError:
...
class PPOAgent(Agent):
    ...
    def __init__(
        ...
        if len(game_inputs) > 1:
            raise bluetangError("PPOAgent only supports a single axis of game inputs.")

        if game_inputs[0]["control_type"] != InputControlTypes.DISCRETE:
            raise bluetangError("PPOAgent only supports discrete input spaces")
