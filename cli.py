...
from bluetang.utilities import (
    clear_terminal,
    display_bluetang_logo,
    is_windows,
    is_linux,
)
...
@click.command(help="Perform bluetang.AI setup")
def setup():
    clear_terminal()
    display_bluetang_logo()

    print("")
    print("bluetang.AI Setup")
    print("")
    ...
    if data_path.exists():
        confirm = input(
            """It appears this machine has already been set up to use bluetang.AI.
...
(One of: 'YES', 'NO') """
    ...
    print("Creating bluetang.AI data directory...")
    data_path.mkdir()

    print("Populating bluetang.AI data directory...")
    ...
    shutil.copy(
        source_path.joinpath("bluetang/config/config.json"),
        data_path.joinpath("config.json"),
    )

    print("")
    print("bluetang.AI Setup Complete!")


@click.command(help="Update bluetang.AI to the latest version")
def update():
    import shlex
    ...
    print("Updating bluetang.AI to the latest version...")
    print("")

    subprocess.call(shlex.split("pip install --upgrade bluetangAI"))
...
@click.command(help="Launch the bluetang.AI GUI")
def gui():
    # TODO: Implement
    pass
...
def download(module):
    ...
    if module == "tesseract":
        if is_windows():
            ...
            _download_module(
                "https://github.com/bluetangAI/bluetangAI/releases/download/optional/tesseract_4.00.00a_win_amd64.zip",
...
# SDK
# These commands are aimed at developers wanting to create plugins for bluetang.AI
@click.command(help="SDK - Perform bluetang.AI SDK setup in the current directory")
def sdk_setup():
    import bluetang.ocr
    ...
    if not have_tesseract:
        ...
        if is_windows():
            print(
                "For an easy installation of Tesseract, run 'bluetang download tesseract"
            )

    # Has setup already been performed?
    if pathlib.Path(".bluetang-sdk").is_file():
        confirm = input(
            """The current directory has already been set up to use the bluetang.AI SDK.
...
(One of: 'YES', 'NO') """
    ...
    shutil.copy(source_path.joinpath("bluetang/config/config_sdk.json"), config_path)
    ...
    # Dot File
    open(".bluetang-sdk", "w").close()

    print("")
    print("bluetang.AI SDK Setup Complete!")
...
@click.command(help="SDK - CUDA test for bluetang.AI")
def sdk_test_cuda():
    import torch
    ...
    if torch.cuda.is_available():
        print("Success! CUDA can be used by bluetang.AI")
    else:
        print("Failure! CUDA cannot be used by bluetang.AI")


@click.command(help="SDK - Test bluetang.AI input capture")
...
def _get_data_path():
    if is_windows():
        data_path = pathlib.Path(os.getenv("APPDATA")).joinpath("bluetang.AI")
    elif is_linux():
        data_path = pathlib.Path(os.getenv("HOME")).joinpath(".bluetang")

    return data_path.absolute()


@click.group(invoke_without_command=True)
@click.option("--version", help="Shows bluetang.AI version", is_flag=True)
@click.option("--help", help="Shows bluetang.AI CLI commands", is_flag=True)
@click.pass_context
